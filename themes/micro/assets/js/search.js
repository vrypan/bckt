(function () {
  ...
  function renderFromTemplate(result, tokens) {
    ...
    const isNote = (result.type || '').toLowerCase() === 'note';
    const card = clone.querySelector('[data-search-card]') || clone.firstElementChild;
    if (card) {
      card.classList.toggle('post-summary--note', isNote);
    }
    ...
    if (dateNode) {
      ...
      dateNode.setAttribute('aria-label', `Open result from ${display}`);
    }
